"""
Normal GRIT training with HyCoCLIP Repulsion loss using GLOO backend.
Uses GLOO backend instead of NCCL to avoid timeout issues on HPC clusters.
"""
...
from hycoclip.models import HyCoCLIP_Repulsion
...
model = L(HyCoCLIP_Repulsion)(
    ...
    use_boxes=True,
    use_hierarchies=False,  # Not using Deep-GRIT hierarchies
    loss_fn="hycoclip_loss_repulsion",  # Use HyCoCLIP loss with repulsion term
)
...
train = dict(
    ...
    ddp=dict(  # options for DistributedDataParallel
        broadcast_buffers=False, static_graph=True, backend="gloo"
    ),
