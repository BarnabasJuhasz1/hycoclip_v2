...
@torch.autocast(device_type=_device_type, dtype=_cast_dtype, enabled=_enable_autocast)
def hyco_reweight_loss(image_feats, text_feats, box_image_feats, box_text_feats, all_image_feats, all_text_feats, hierarchy_feats, pairwise_scores, _curv, _rank, _scale, entail_weight=1.0, hier_distance_weight=1.0):
    """
    ...
    hier_logits_1 = L.elementwise_dist(hierarchy_feats[0], hierarchy_feats[1], _curv)
    hier_logits_2 = L.elementwise_dist(hierarchy_feats[1], hierarchy_feats[2], _curv)
    hier_logits_3 = L.elementwise_dist(hierarchy_feats[2], hierarchy_feats[3], _curv)
    hier_logits_4 = L.elementwise_dist(hierarchy_feats[3], hierarchy_feats[4], _curv)
    ...
    # Hyperbolic entailment loss: text should entail matching image.
    _angle = L.oxy_angle(text_feats, image_feats, _curv)
    _aperture = L.half_aperture(text_feats, _curv, min_radius=0.05)

    _box_angle = L.oxy_angle(box_text_feats, box_image_feats, _curv)
    _box_aperture = L.half_aperture(box_text_feats, _curv, min_radius=0.05)

    _cross_image_angle = L.oxy_angle(box_image_feats, image_feats, _curv)
    _box_image_aperture = L.half_aperture(box_image_feats, _curv, min_radius=0.05)

    _cross_text_angle = L.oxy_angle(box_text_feats, text_feats, _curv)
    _box_text_aperture = L.half_aperture(box_text_feats, _curv, min_radius=0.05)

    # angle chain between each consecutive element in the hierarchy
    hier_chain_angle_1 = L.oxy_angle(hierarchy_feats[1], hierarchy_feats[0], _curv)
    hier_chain_angle_2 = L.oxy_angle(hierarchy_feats[2], hierarchy_feats[1], _curv)
    hier_chain_angle_3 = L.oxy_angle(hierarchy_feats[3], hierarchy_feats[2], _curv)
    hier_chain_angle_4 = L.oxy_angle(hierarchy_feats[4], hierarchy_feats[3], _curv)

    # apertures of hierarchy entries
    hier_aperture_1 = L.half_aperture(hierarchy_feats[1], _curv)
    hier_aperture_2 = L.half_aperture(hierarchy_feats[2], _curv)
    hier_aperture_3 = L.half_aperture(hierarchy_feats[3], _curv)
    hier_aperture_4 = L.half_aperture(hierarchy_feats[4], _curv)
    ...
    loss = contrastive_loss
    loss = loss + entail_weight * entailment_loss
    loss = loss + hier_distance_weight * mean_elementwise_dist
