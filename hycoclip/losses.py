...
@torch.autocast(device_type=_device_type, dtype=_cast_dtype, enabled=_enable_autocast)
def hycoclip_deep_loss_repulsion_poly(image_feats, text_feats, box_image_feats, box_text_feats, hierarchy_feats_list, hier_sample_type, all_image_feats, all_text_feats, _curv, _rank, _scale, entail_weight=1.0, repulsion_weight=0.1, repulsion_r0=0.5):
    """
    Computes the HyCoCLIP Deep loss with polynomial repulsion term.
    Repulsion loss: repulsion_weight * max(0, r0 - r)^2
    where r is the norm of box embeddings and r0 is the target minimum norm (default 0.5).
    ...
    When r >= r0: no penalty (embeddings are already far enough)
    """
    # Get standard HyCoCLIP Deep loss
    base_loss = hycoclip_deep_loss(image_feats, text_feats, box_image_feats, box_text_feats,
                                   hierarchy_feats_list, hier_sample_type,
                                   all_image_feats, all_text_feats, _curv, _rank, _scale, entail_weight)

    # Compute polynomial repulsion loss: max(0, r0 - r)^2
    # r = norm of embeddings
    # The hierarchy embeddings are deliberately left out: in the entailment cones
    # a small norm is what makes a term general, so repelling them would fight
    # the abstraction ladder the deep loss is trying to build.
    image_norms = torch.norm(image_feats, dim=-1)  # (batch_size,)
    text_norms = torch.norm(text_feats, dim=-1)    # (batch_size,)
    box_image_norms = torch.norm(box_image_feats, dim=-1)  # (batch_size,)
    box_text_norms = torch.norm(box_text_feats, dim=-1)    # (batch_size,)
...
@torch.autocast(device_type=_device_type, dtype=_cast_dtype, enabled=_enable_autocast)
def hyco_reweight_loss_per_sample_repulsion_poly(image_feats, text_feats, box_image_feats, box_text_feats, all_image_feats, all_text_feats, hierarchy_feats, pairwise_scores, _curv, _rank, _scale, entail_weight=1.0, contrast_weight=1.0, repulsion_weight=0.1, repulsion_r0=0.5):
    """
    Computes the HyCoCLIP ReWeight loss with polynomial repulsion term.
    Repulsion loss: repulsion_weight * max(0, r0 - r)^2
    where r is the norm of box embeddings and r0 is the target minimum norm (default 0.5).
    ...
    When r >= r0: no penalty (embeddings are already far enough)
    """
    # Get standard HyCoCLIP ReWeight loss
    base_loss = hyco_reweight_loss_per_sample(image_feats, text_feats, box_image_feats, box_text_feats,
                                   all_image_feats, all_text_feats, hierarchy_feats, pairwise_scores,
                                   _curv, _rank, _scale, entail_weight, contrast_weight)

    # Compute polynomial repulsion loss: max(0, r0 - r)^2
    # r = norm of embeddings
    # The hierarchy embeddings are deliberately left out: in the entailment cones
    # a small norm is what makes a term general, so repelling them would fight
    # the abstraction chain the re-weight loss is trying to build.
    image_norms = torch.norm(image_feats, dim=-1)  # (batch_size,)
    text_norms = torch.norm(text_feats, dim=-1)    # (batch_size,)
    box_image_norms = torch.norm(box_image_feats, dim=-1)  # (batch_size,)
    box_text_norms = torch.norm(box_text_feats, dim=-1)    # (batch_size,)
...
@torch.autocast(device_type=_device_type, dtype=_cast_dtype, enabled=_enable_autocast)
def hyco_reweight_loss_repulsion_poly(image_feats, text_feats, box_image_feats, box_text_feats, all_image_feats, all_text_feats, hierarchy_feats, pairwise_scores, _curv, _rank, _scale, entail_weight=1.0, contrast_weight=1.0, repulsion_weight=0.1, repulsion_r0=0.5):
    """
    Computes the HyCoCLIP ReWeight loss with polynomial repulsion term.
    Repulsion loss: repulsion_weight * max(0, r0 - r)^2
    where r is the norm of box embeddings and r0 is the target minimum norm (default 0.5).
    ...
    """
    # Get standard HyCoCLIP ReWeight loss
    base_loss = hyco_reweight_loss(image_feats, text_feats, box_image_feats, box_text_feats,
                                   all_image_feats, all_text_feats, hierarchy_feats, pairwise_scores,
                                   _curv, _rank, _scale, entail_weight, contrast_weight)
