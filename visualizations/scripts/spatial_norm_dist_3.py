...
def main(_A: argparse.Namespace):
    ...
    for batch in tqdm(dataloader, desc=f"Generating representation norms"):
        ...
        with torch.inference_mode():
            ...
            hierarchy_tokens = [tokenizer(hier) for hier in batch["text_hierarchy"]]
    ...
    all_image_norms = np.concatenate([image_norms, box_image_norms], axis=0)
    all_text_norms = np.concatenate([text_norms, box_text_norms], axis=0)
    logger.info(f"Modality Gap Without Hierarchies ( |Image - Text| ) Norm, Mean: {np.abs((all_image_norms - all_text_norms).mean()):.4f}")


    logger.info(f"Hierarchy level 0 norm, Mean: {hier_norms_0.mean():.4f}, Min: {hier_norms_0.min():.4f}, Max: {hier_norms_0.max():.4f}")
    ...
    logger.info(f"Hierarchy level 3 (general) norm, Mean: {hier_norms_3.mean():.4f}, Min: {hier_norms_3.min():.4f}, Max: {hier_norms_3.max():.4f}")
    # logger.info(f"Hierarchy level 4 (general) norm, Mean: {hier_norms_4.mean():.4f}, Min: {hier_norms_4.min():.4f}, Max: {hier_norms_4.max():.4f}")
    
    all_text_norms_with_hier = np.concatenate([text_norms, box_text_norms, hier_norms_0, hier_norms_1, hier_norms_2, hier_norms_3], axis=0)
    average_text = all_text_norms_with_hier.mean()
    average_image = all_image_norms.mean()
    logger.info(f"Modality Gap Including Hierarchies ( |Image - Text| ) Norm, Mean: {(average_image - average_text):.4f}")

    # check chamfer distance
    ...
    sns.histplot(data=hier_norms_0.squeeze(), bins='auto', stat="percent", kde=True, element="step", alpha=0.5, label='H0')
    ...
    sns.histplot(data=hier_norms_3.squeeze(), bins='auto', stat="percent", kde=True, element="step", alpha=0.5, label='H3 (general)')
