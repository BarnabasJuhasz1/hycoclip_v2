...
class GroundedDatasetTarMapper:
    ...
    def __init__(
        ...
        self.key_to_hierarchy = {}
        self.hierfile_count = 0
        for path in hierfiles:
            for file in glob.glob(path):
                with open(file, "r") as f:
                    json_data = json.load(f)
                    self.key_to_hierarchy.update(json_data)
            self.hierfile_count += len(glob.glob(path))

        logger.info(f"{self.__class__.__name__} found {self.hierfile_count} JSONs and {len(self.key_to_hierarchy)} hierarchies.")
    ...
    def __call__(self, dataset_dict: dict):
        ...
        parent_content_hier = self.key_to_hierarchy.get(key, None)[parent_id]
        if parent_content_hier in {None, {}}:
            hierarchy = None # fallback to None if missing
        else:
            hierarchy = parent_content_hier["original"]
